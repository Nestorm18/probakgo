...
func (s *Store) GetEmailConfig(ctx context.Context) (*domain.EmailConfig, error) {
	debug.RecordQuery(ctx, `SELECT id, smtp_host, smtp_port, smtp_user, smtp_password, recipients, is_enabled, send_time, retention_months, retention_enabled, alert_disk_pct, alert_backup_err, alert_pbs_stale_hours, public_api_url, alert_pve_heartbeat_minutes FROM email_config LIMIT 1`)
...
func (s *Store) UpsertEmailConfig(ctx context.Context, c domain.EmailConfig) error {
	debug.RecordQuery(ctx, `INSERT INTO email_config (id, smtp_host, smtp_port, smtp_user, smtp_password, recipients, is_enabled, send_time, retention_months, retention_enabled, alert_disk_pct, alert_backup_err, alert_pbs_stale_hours, public_api_url, alert_pve_heartbeat_minutes) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET ...`)
